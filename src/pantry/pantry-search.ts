...
import styles from './pantry-search.css'
...
function initLunr(pantry) {
	function createDocsRecurse(pantry, ns = ''): {
		iid: string,
		name?: string,
		category?: string,
		qty?: string,
		...
		return Object.entries(pantry).map(([name, obj]) => {
...
export const PantrySearch: Hybrids<Element> = {
	...
	render: ({search}) => html`<cam-box p="0 2" class="pantry-search">
